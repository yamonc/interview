...
MySQL整体来看，可以大致分为两个部分，第一是server层，主要是MySQL功能上的事情，还有一层是引擎层，负责存储相关的具体事宜。redo log是属于InnoDB引擎特有的日志。binlog是server层的日志。
...
redo log是物理日志，记录的是在某个数据页上做了什么修改。binlog是逻辑日志，记录的是这个语句的原始逻辑，比如给id=2的这一行的c字段+1

InnoDB中的redo log是固定大小的，从头开始写，写到末尾之后回到开头循环写，比如这样：

![image-20210916101351937](https://gitee.com/yamonc/blogImage/raw/master//img/blogImage/image-20210916101351937.png)

write pos表示当前记录的位置，一边写一边后移，写到3号文件末尾之后就要回到0号文件开头。

checkpoint表示当前要擦除的位置，也是向后移动并且循环的，擦除记录前要把记录更新到数据文件。

显然，write pos和checkpoint之间就是未写的部分，可以用来记录新的操作。如果write pos追上了checkpoint，表示记录满了，不能执行更新，需要擦除一些记录，将checkpoint向前推进一点。

crash-safe：表示如果数据库突然崩溃了，数据库重启的时候，可以根据redo log来恢复数据库中的内容。
...
1. redo log是InnoDB引擎层特有的log日志，binlog 是MySQLserver层实现的，所有引擎都可以用。
2. redo log是物理日志，记录的是在某个页面上做了什么修改；binlog是逻辑日志，记录的是这个语句的原始逻辑，比如给id=2的这一行的c字段+1；
3. redo log是循环写，空间固定可以用完；binlog是追加写，写到一定大小之后，会切换到下一个，不会覆盖之前的记录。

### 一条更新语句是怎么执行的？

假如执行update id=2这条语句。

1. 如果有索引的话，优化器会优先执行索引查找，执行器找到id=2的这一行，如果id=2这一行所在的数据页本来就在内存中，就直接返回给执行器。否则需要先从磁盘读入内存，然后再返回。
2. 执行器拿到引擎的行数据，然后+1，比如是N，现在N+1，得到新的一行数据，再调用引擎接口写入这行新数据。
3. 引擎将这行新数据更新到内存中，同时将这个更新操作记录到redo log中，此时redo log处于prepare状态。然后通知执行器执行完成，可以提交事务。
4. 执行器生成这个操作的binlog，然后将binlog写入磁盘。
5. 执行器调用引擎的提交事务接口，引擎把刚刚写入的redo log改成commit状态，更新完成。

图示：

![image-20210916103550385](https://gitee.com/yamonc/blogImage/raw/master//img/blogImage/image-20210916103550385.png)

这里将redo log的写入拆分成了两个步骤：prepare和commit两阶段提交。

### 数据库崩溃后怎么恢复的？

通过binlog恢复。首先系统定期做整库备份。得到恢复的时间，然后找到最近的一次全量备份，从这个备份恢复到临时库，然后从备份的时间点开始，将备份的binlog依次取出，重放到误删表之前的那个时刻。这个时候临时库就和误删之前的线上库是一致的了，然后可以把表数据从临时库中取出来，按需要恢复到线上库。
...
![image-20210916110301126](https://gitee.com/yamonc/blogImage/raw/master//img/blogImage/image-20210916110301126.png)



为了让这两份日志之间的逻辑保持一致，保证数据库崩溃恢复之后不出现问题。假如正在更新一个数据，假设id=2，更新之前字段c为0，更新为1.

1. 如果先写redo log后写binlog，假设在redo log写完之后，binlog还没有写完，MySQL异常终止。redolog写完之后，即使系统崩溃了，仍然可以将数据恢复回来，所以恢复后的这一行c为1. 但是由于binlog还没写完就crash了，所以binlog没有这个语句，之后备份日志的时候存起来的binlog没有这条语句，没有这条语句的后果是如果使用binlog恢复临时库的时候，由于丢失这条语句，少了这次更新，所以就会出现这行的值为0，跟原库不一致。
2. 先写binlog后写redo log。如果在binlog写完之后crash了，由于redo log还没有写，崩溃恢复以后这个事务无效，所以这一行的c值为0，但是binlog里面已经记录了“c从0改成1”。所以使用binlog恢复的时候多出来一个事务，这个事务恢复一行c的值为1，跟原库不同。
...
![image-20210916133617267](https://gitee.com/yamonc/blogImage/raw/master//img/blogImage/image-20210916133617267.png)

参考：[全网最牛X的！！！MySQL两阶段提交串讲](https://zhuanlan.zhihu.com/p/343449447)

### 如何理解MVCC？

MVCC的形式是非阻塞读，对于select、show等sql 的时候并不加锁，当涉及到insert、update等修改的时候会加排他锁。MVCC有以下特点：

- 每行数据都存在一个版本，每次数据更新时都更新该版本
- 修改时Copy出当前版本随意修改，个事务之间无干扰
- 保存时比较版本号，如果成功（commit），则覆盖原记录；失败则放弃copy（rollback）就是每行都有版本号，保存时根据版本号决定是否成功，听起来含有乐观锁的味道。。。，而Innodb的实现方式是：事务以排他锁的形式修改原始数据
- 把修改前的数据存放于undo log，通过回滚指针与主数据关联修改成功（commit）啥都不做，失败则恢复undo log中的数据（rollback） 
...
![image-20210916211839990](https://gitee.com/yamonc/blogImage/raw/master//img/blogImage/image-20210916211839990.png)
...
前台中的通知公告模块：主要用于展示管理员在后台填写的通知公告、培训信息。

前台中首页的信息显示比如显示通知公告的最新五条、培训信息的最新五条、最新通过创新工程师的五个人、最新通过创新案例。

后台管理员中心：

公告管理：公告的定义是新闻信息和培训信息。新闻信息主要是宣传创新工程师的开展工作，培训信息主要是用来宣布创新工程师的培训信息，比如开班信息、培训公告等。这块的内容主要就是对公告的增删改查。

创新工程师培训模块中的课题选题管理：用户在前台填写了培训申请后，需要填写相应的课题，如果用户的培训申请通过了之后，可以在这查看对应的课题（但是我们并没有这么做，这里显示的是所有人的培训过程的课题）。管理员对课题进行增删改查，可以导出所有课题或者选中的课题，也可以删除选中的课题。

创新工程师申请模块：创新工程师模块中的认证成绩录入、认证证书录入两个模块。这两个模块的主要流程是，用户在前台填写了创新工程师的申请表，并且成功提交了认证的响应条件之后，管理员在后台对材料进行审核，如果审核成功之后，进行线下笔试和面试，管理员需要根据线下的笔试面试成绩标记该用户是否通过，默认是通过状态。如果该用户通过了则进入下一个流程，录入证书。管理员给通过的用户输入证书编号和上传证书（照片）到服务器，也可以导出所有通过的用户。

创新工程师信息管理模块：这个模块主要是包含了所有申请创新工程师的信息，不仅是通过的创新工程师，也有未通过的创新工程师，这样做的目的是为了更大化信息范围。管理员可以在这里查看创新工程师的信息。

案例管理模块中的难题管理：难题管理主要分已通过的认证学院的案例，未通过的认证学员的案例。在这里管理员对这些案例只有删除查看的权限，可以批量导出全版、简版案例也可以导出单个案例。为了用户操作的安全性，我们只设置了删除单个的案例的情况。

IKC案例管理：用户可以申请别的平台的账号，然后可以在别的平台上填写案例，我这块在接收过来用户在另外一个平台填写的所有案例。管理员可以在这里查看查找案例，导出案例。
...
1. 可以使用分页的方式，将100w个文件分割成为1000份，每份1000个文件，这样使用1000份
...
因为每个页面几乎都会涉及到搜索，如果以后用户量上来了之后，有可能会成为整个系统的瓶颈之一，所以可以将搜索定制为搜索微服务模块，专门开设端口和服务器。
